...
(function () {
    ...
    function sensible_unit(x, units) {
        ...
        for(var i=0; i<units.length; i++) {
            if (x<1e3) { return my_round(x).toString()+" "+units[i]; }
            x /= 1e3;
        }
    ...
    var regexs = [], replacements = [],
        tagsWhitelist = ['PRE', 'BLOCKQUOTE', 'CODE', 'INPUT', 'BUTTON', 'TEXTAREA', 'SPAN'],
        rIsRegexp = /^\/(.+)\/([gim]+)?$/,
        word, text, texts, i, userRegexp;

    // function to decide whether a parent tag will have its text replaced or not
    function isTagOk(tag) { return tagsWhitelist.indexOf(tag) === -1; }
